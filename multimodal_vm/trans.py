    ...
    def tf(self, img, k=0):
        ...
        for bs in range(bsize):
            if k == 0:
                ...
                channels = [
                    rotate(img[bs, :, :, :], self.angle_buffer, axes=self.axes_buffer, reshape=False, order=0,
                           mode='constant', cval=-1)]
                img[bs, ...] = np.stack(channels)
...
class Resize_img(Base):
    ...
    def tf(self, img, k=0):
        if k == 1:
            img = resize(img, (img.shape[0], img.shape[1], self.shape[0], self.shape[1], self.shape[2]),
...
class GaussianBlur(Base):
    def __init__(self, dim, sigma=Constant(0.5), app=-1):
        # 1.5 pixel
        self.dim = dim
        self.sigma = sigma
        self.eps = 0.001
